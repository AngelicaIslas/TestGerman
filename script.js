...
function startLearningPhase() {
    ...
    if ((groupInput === "1" || groupInput === "2" || groupInput === "3") && personalCodeInput) {
        ...
        alert("Bitte geben Sie eine gültige Gruppennummer (1, 2 oder 3) und Ihren persönlichen Code ein.");
    }
}
...
function playAudioSequence() {
    const audioQueue = [
        { category: "Kardial", priority: "Hoch", src: getAudio("Cardiovascular", "High") },
        { category: "Kardial", priority: "Mittel", src: getAudio("Cardiovascular", "Medium") },
        { category: "Beatmung", priority: "Hoch", src: getAudio("Ventilation", "High") },
        { category: "Beatmung", priority: "Mittel", src: getAudio("Ventilation", "Medium") },
        { category: "Medikamenten- oder Flüssigkeitsgabe", priority: "Hoch", src: getAudio("DrugAdministration", "High") },
        { category: "Medikamenten- oder Flüssigkeitsgabe", priority: "Mittel", src: getAudio("DrugAdministration", "Medium") }
    ];
...
function playSequence(queue, repetitions, callback) {
    ...
    function playNext() {
        ...
        function playAudio() {
            ...
            document.getElementById("design").textContent = currentDesign === "icons" ? "Icons" : "Melodisch";//currentDesign === "icons" ? "Icons" : "Melodic";
...
function getRandomAlarm(design, excludeCategory) {
    ...
    // const randomPriority = priorities[Math.floor(Math.random() * priorities.length)]; exchanged for the option to let exclude prios 
...
document.getElementById("confirm-selection").addEventListener("click", () => {
    ...
    if (alarm1Category && alarm1Priority && alarm2Category && alarm2Priority) {
        ...
        alert("Bitte treffen Sie eine gültige Auswahl."); //alert("Please make valid selections.");
    }
});
...
document.getElementById("finish-test").addEventListener("click", () => {
    exportToCSV();  
    document.getElementById("test-phase").innerHTML = "<h2>Test abgeschlossen</h2><p>Danke fürs Mitmachen!</p><p>Sie können Ihren Browser jetzt schließen.</p>";
});
